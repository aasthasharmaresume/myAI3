...
export default function Chat() {
  ...
  // persist to localStorage whenever messages/durations change on client
  useEffect(() => {
    if (isClient) {
  ...
  useEffect(() => {
    if (
      ...
      const welcomeMessage: UIMessage = {
        id: `welcome-${Date.now()}`,
        role: "assistant",
        // ✅ use `content` so it renders in your JSX
          ...
          <div className="chat-window w-full max-w-3xl flex flex-col gap-4 h-[calc(100vh-7rem)]">
            ...
            <div className="flex-1 overflow-y-auto">
              {messages.map((m: any) => (
                ...
                >
                  {/* Support both `content` and `parts` just in case */}
            ...
            {/* INPUT BAR (single form using RHF) */}
            <form
